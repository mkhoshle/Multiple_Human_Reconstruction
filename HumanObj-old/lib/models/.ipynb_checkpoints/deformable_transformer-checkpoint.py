...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, window_srcs, window_masks, window_pos_embeds): 
        ...
        # NOTE: IF THIS IS THE SAME AS ABOVE, REMOVE
        # prepare input for decoder 
        spatial_shapes_de = []
        for lvl, (window_src, window_mask, window_pos_embed) in enumerate(zip(window_srcs, window_masks, window_pos_embeds)):
            bs, c, h, w = window_src.shape
            spatial_shape = (h, w)
            spatial_shapes_de.append(spatial_shape)
            
        spatial_shapes_de = torch.as_tensor(spatial_shapes_de, dtype=torch.long, device=src_flatten.device)
        valid_ratios_de = torch.stack([self.get_valid_ratio(m) for m in window_masks], 1)
        
        # decoder
        query_attn_mask = None
        hs, inter_references = self.decoder(
            tgt, spatial_shapes_de, valid_ratios_de, src, src_spatial_shapes, src_valid_ratios,
            pos_embeds, window_pos_embeds, mask_flatten, query_attn_mask)
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, pos, encoder_pos, reference_points, src, src_spatial_shapes, src_padding_mask=None, query_attn_mask=None):
        # self attention
        q = k = self.with_pos_embed(tgt, encoder_pos)
        ...
        # cross attention
        tgt2 = self.cross_attn(self.with_pos_embed(tgt, pos),
                               reference_points,
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, tgt_spatial_shapes, tgt_valid_ratios, src, src_spatial_shapes, src_valid_ratios,
                pos=None, encoder_pos=None, src_padding_mask=None, query_attn_mask=None):
        reference_points = self.get_reference_points(tgt_spatial_shapes, tgt_valid_ratios, device=src.device)
        ...
        for lid, layer in enumerate(self.layers):
            ...
            output = layer(output, pos, encoder_pos, reference_points_input, src, src_spatial_shapes, src_padding_mask, query_attn_mask)
