...
class Internet(Dataset):
    ...
    def get_image_from_video_name(self,imgpath): 
        ...
        shape = image.shape
        image = torch.from_numpy(image).float().contigious().view(shape[2],shape[0],shape[1])
    ...
    def get_item_single_frame(self,index):
        ...
        shape = image.shape
        # image = torch.from_numpy(image).float().contiguous().view(shape[2],shape[0],shape[1])
    ...
    def get_prev_image(self, imgpath, input_size=512, ds='internet', single_img_input=False): 
        print(imgpath)
        image = cv2.imread(imgpath)
        image = torch.from_numpy(image).float().contiguous()
