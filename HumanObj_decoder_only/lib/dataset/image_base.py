...
class Image_base(Dataset):
    ...
    def get_image_from_video_name(self,imgpath): 
        ...
        # dst_image = dst_image.reshape(shape[2],shape[0],shape[1])
    ...
    def get_item_single_frame(self,index):
        ...
        input_data = {
            ...
            'data_set': info['ds'],
            'data_class': info['data_class'] # Video_name
        }

        # print(input_data['image'].shape, input_data['image_org'].shape)
...
def test_dataset(dataset,with_3d=False,with_smpl=False):
    ...
    visualizer = Visualizer(resolution = (512,512,3), result_img_dir=save_dir,renderer_type='pytorch3d')
