    ...
    for iter_num, meta_data in enumerate(loader_val):
        ...
        for ds in set(outputs['meta_data']['data_set']):
            ...
            if args().calc_PVE_error and ds in constants.PVE_ds:
                target_theta = torch.cat([outputs['meta_data']['params'][val_idx, :66].cpu(), torch.zeros(
                    len(val_idx), 6), outputs['meta_data']['params'][val_idx, 66:].cpu()], 1)
                pred_theta = torch.cat([outputs['params']['global_orient'],
                                       outputs['params']['body_pose'], outputs['params']['betas']], 1)
                ED['PVE'][ds]['target_theta'].append(target_theta)
                ED['PVE'][ds]['pred_theta'].append(
                    pred_theta[val_idx].float().detach().cpu())

            abs_error, aligned_poses = calc_mpjpe(
                real_3d, predicts, align_inds=align_inds, return_org=True)
            abs_error = abs_error.float().cpu().numpy()*1000
            rt_error = calc_pampjpe(
                real_3d, predicts).float().cpu().numpy()*1000

            ED['MPJPE'][ds].append(abs_error.astype(np.float32))
            ED['PA_MPJPE'][ds].append(rt_error.astype(np.float32))
            ED['imgpaths'][ds].append(
                np.array(outputs['meta_data']['imgpath'])[val_idx])
            error_dict['3d']['error'].append(abs_error)
            error_dict['3d']['idx'].append(val_idx)

        if iter_num % self.val_batch_size == 0:
            print('{}/{}'.format(iter_num, len(loader_val)))
            MPJPE_result, PA_MPJPE_result, eval_matrix = print_results(
                ED.copy())
            if not evaluation:
                outputs = self.network_forward(
                    eval_model, meta_data_org, window_meta_data, self.val_cfg)
            vis_ids = np.arange(
                max(min(self.val_batch_size, len(outputs['reorganize_idx'])), 8)//8), None
...
def print_results(ED):
    ...
    if len(ED['MPJPE']['h36m']) > 0:
        ...
        PA_MPJPE_acts = h36m_evaluation_act_wise(np.concatenate(ED['PA_MPJPE']['h36m'], axis=0), np.concatenate(
            np.array(ED['imgpaths']['h36m']), axis=0), constants.h36m_action_names)
        MPJPE_acts = h36m_evaluation_act_wise(np.concatenate(ED['MPJPE']['h36m'], axis=0), np.concatenate(
            np.array(ED['imgpaths']['h36m']), axis=0), constants.h36m_action_names)
...
def _init_error_dict():
    ED = {}
    ED['MPJPE'], ED['PA_MPJPE'], ED['PCK3D'], ED['imgpaths'] = [
        {ds: [] for ds in constants.dataset_involved} for _ in range(4)]
    ED['PVE'] = {ds: {'target_theta': [], 'pred_theta': []}
                 for ds in constants.PVE_ds}
    return ED
