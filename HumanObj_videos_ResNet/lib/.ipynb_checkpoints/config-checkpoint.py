...
project_dir = currentfile.replace(os.sep + 'HumanObj_videos_ResNet' + os.sep + 'lib' + os.sep + 'config.py','')
...
def parse_args(input_args=None):
    ...
    train_group.add_argument('--lr_backbone', help='lr_backbone', default=3e-4,type=float)
    ...
    train_group.add_argument('--epoch', type = int, default = 50, help = 'training epochs')
    # train_group.add_argument('--partial_load',type = bool, default = True, help = 'whether to partially load the model')
    ...
    train_group.add_argument('--dropout', default=1e-1, help='Dropout applied in the transformer', type=float)
    train_group.add_argument('--pre_norm', type = bool, default = False)
    train_group.add_argument('--backbone_num_channels', type = int, default = 64, help = 'backbone num of channels')

    model_group = parser.add_argument_group(title='model settings')
    # model settings
    model_group.add_argument('--track_attention', type = bool, default = False)
    ...
    model_group.add_argument('--masks',type = bool,default = False ,help = 'Segmentation')
    model_group.add_argument('--dilation',type = bool,default = False ,help = 'If true, we replace stride with dilation in the last convolutional block (DC5)')
    model_group.add_argument('--hidden_dim', type = int, default = 256, help = 'Size of the embeddings i.e. dimension of the transformer')
    model_group.add_argument('--hidden_dim_pos', type = int, default = 256, help = 'n_steps for positional encoding')
    model_group.add_argument('--position_embedding', type = str, default = "sine", help = '# Type of positional embedding to use on top of the image features: sine,learned')
    
    model_group.add_argument('--backbone',type = str,default = 'resnet50',help = 'backbone model: resnet50, resnet101')
    ...
    model_group.add_argument('--resnet_pretrain', type=str, default= os.path.join(project_dir,'trained_models/resnet101-5d3b4d8f.pth'))
    ...
    #dataset setting:
    dataset_group.add_argument('--dataset_rootdir',type=str, default=os.path.join(root_dir,'dataset/ROMP_datasets'), help= 'root dir of all datasets')
    ...
    smpl_group.add_argument('--smpl_model_path',type = str,default = os.path.join(model_dir,'parameters/smpl'),help = 'smpl model path')
    smpl_group.add_argument('--smpl_J_reg_h37m_path',type = str,default = os.path.join(model_dir, 'parameters/smpl', 'J_regressor_h36m.npy'),help = 'SMPL regressor for 17 joints from H36M datasets')
    smpl_group.add_argument('--smpl_J_reg_extra_path',type = str,default = os.path.join(model_dir, 'parameters/smpl', 'J_regressor_extra.npy'),help = 'SMPL regressor for 9 extra joints from different datasets')
    
    smpl_group.add_argument('--smpl_uvmap',type = str,default = os.path.join(model_dir, 'parameters/smpl', 'smpl_vt_ft.npz'),help = 'smpl UV Map coordinates for each vertice')
