...
class HOBJ(Base):
    def __init__(self, backbone=None, **kwargs):
        ...
        self._build_model()
        self.input_proj = nn.Conv2d(256, 64, kernel_size=1)
        # self.input_proj = nn.Conv2d(64, 64, kernel_size=8,stride=8)
    ...
    def head_forward(self,meta_data, window_meta_data):
        ...
        batch_size, _, h, w = meta_data.shape
        window_meta_data = window_meta_data.contiguous().cuda()
    
        src = self.backbone(meta_data)['pool'] 
        ...
        window_src = self.backbone(window_meta_data)['pool']    
