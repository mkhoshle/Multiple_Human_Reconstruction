...
from lib.utils.fusion import Laplacian_pyramid
...
class Trainer(Base):
    def __init__(self):
        ...
        self.N_frames_per_window = args().N_frames_per_window
    ...
    def get_prev_frame(self, frame_index, N_frames_per_window):
        frames_list = []
        for i in range(self.N_frames_per_window):
            if frame_index-10 >= 0:
    ...
    def get_frame_index(self, root, data_class, img_name):
        if data_class == 'pw3d':
            ...
            prev_frames = self.get_prev_frame(frame_index, self.N_frames_per_window)
            
            prev_frames_path = [os.path.join(root, "image_"+"0"*(5-len(str(prev_frame)))+str(prev_frame)+".jpg")
    ...
    def get_window(self, img_info, **kwargs):
        ...
        for i, path in enumerate(img_info['imgpath']):
            ...
            prev_frames_path = self.get_frame_index(root,
                                                img_info['data_class'][i], path[-1])

            prev_images = [dataset.get_image_from_video_name(prev_frame_path) for prev_frame_path in prev_frames_path]
            if args().fusion:
                window_features = [self.model.backbone(prev_image.unsqueeze(0).contiguous().cuda()).squeeze(0)
                                   for prev_image in prev_images]
                fused_image = Laplacian_pyramid(window_features)
                prev_image = fused_image
            else:
                window_features = [self.model.backbone(prev_image) for prev_image in prev_images]
                prev_image = window_features
    ...
    def train_epoch(self, epoch):
        ...
        for iter_index, meta_data in enumerate(self.loader):
            ...
            print(window_meta_data.shape,"shape")
